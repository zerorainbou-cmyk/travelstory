(function( $ ) {
	...
	function initializeMap( mapInstance ) {
		...
		var zoom = 17;
		var map_zoom = mapInstance.find( '.pw-map-zoom' ).val();

		if ( map_zoom ) {
			var zoom = parseInt( map_zoom );
		}
	...
	$(document).ready( function() {
	    ...
	    if ( typeof ovabrwCategoriesDetination != 'undefined' ) {
	        var postType    = $('.edit-php.post-type-destination');
	        var titleFirst  = postType.find('.page-title-action');

	        if ( ovabrwCategoriesDetination.url && ovabrwCategoriesDetination.title ) {
	            titleFirst.after(
	                '<a href="' +
	                ovabrwCategoriesDetination.url +
	                '" class="page-title-action">' +
	                ovabrwCategoriesDetination.title +
	                '</a>'
	            );
	        }
	    } // END
