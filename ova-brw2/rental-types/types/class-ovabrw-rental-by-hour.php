...
/**
 * OVABRW Rental By Hour
 */
if ( !class_exists( 'OVABRW_Rental_By_Hour' ) ) {

	class OVABRW_Rental_By_Hour extends OVABRW_Rental_Types {
		...
		public function get_price_html( $price_html = '', $currency = '' ) {
			...
			if ( $sale_price ) {
				...
				$price = (float)$this->get_meta_value( 'regul_price_hour' );
			}
			if ( !$price ) $price = (float)get_post_meta( $this->get_id(), '_regular_price', true );

			// New price
			$new_price = sprintf( esc_html__( '%s / Hour', 'ova-brw' ), ovabrw_wc_price( $price, [ 'currency' => $currency ] ) );
	    ...
	    public function get_calendar_regular_price() {
	    	// init
	    	$regular_price = $this->get_meta_value( 'regul_price_hour' );
	    ...
	    public function get_rental_calculations( $args = [] ) {
	    	...
	    	// Get special prices
	    	$special_prices = $this->get_special_prices( $pickup_date, $dropoff_date, $numberof_rental_hours );
	    	...
	    	if ( $numberof_rental_hours <= $special_qty ) {
	    		...
	    		// Get discount prices
				$discount_prices = $this->get_discount_prices( $numberof_rental_hours - $special_qty );
				...
				// Rental price
				$rental_price = (float)$regular_total + (float)$special_total;
	    	}
	    ...
	    public function get_price_details( $cart_item = [] ) {
	        ...
	        // Base price based on rental hours
	    ...
	    public function get_number_of_rental_hours( $pickup_date, $dropoff_date ) {
	    	...
	    	$numberof_rental_hours = ceil( ( $dropoff_date - $pickup_date ) / 3600 );
	     ...
	     * Get special prices
	     */
	    public function get_special_prices( $pickup_date, $dropoff_date, $numberof_rental_hours ) {
	    	...
	    	return apply_filters( $this->prefix.'get_special_prices', $results, $pickup_date, $dropoff_date, $numberof_rental_hours, $this );
	    }
	     ...
	     * Get discount prices
	     */
	    public function get_discount_prices( $numberof_rental_hours ) {
	    	...
	    	if ( ovabrw_array_exists( $disc_price ) ) {
	    		...
	    		foreach ( $disc_price as $i => $p ) {
	    			...
	    			if ( $numberof_rental_hours >= $from && $numberof_rental_hours <= $to ) {
	    				$price = (float)$p;
	    	...
	    	return apply_filters( $this->prefix.'get_discount_prices', $price, $numberof_rental_hours, $this );
	    }
		...
		public function get_real_data( $pickup_date, $dropoff_date ) {
			...
			if ( $numberof_rental_hours > 0 ) {
				...
				// Get discount prices
				$discount_prices = $this->get_discount_prices( $numberof_rental_hours );
				if ( $discount_prices ) $regular_price = $discount_prices;
		...
		public function get_request_booking_mail_content( $data = [] ) {
    		...
    		if ( $location_prices ) {
    			$order_details .= '<tr>';
    				...
    				$order_details .= '<td>' . esc_html( $location_prices ) . '</td>';
    			$order_details .= '</tr>';
    		}
