...
/**
 * OVABRW Rental By Day
 */
if ( !class_exists( 'OVABRW_Rental_By_Day' ) ) {

	class OVABRW_Rental_By_Day extends OVABRW_Rental_Types {
		...
		public function get_price_html( $price_html = '', $currency = '' ) {
			...
			if ( $sale_price ) {
				...
				$price = (float)$this->get_meta_value( 'regular_price_day' );
			}
			...
			if ( 'hotel' === $charged_by ) {
				...
				$new_price = sprintf( esc_html__( '%s / Day', 'ova-brw' ), ovabrw_wc_price( $price, [ 'currency' => $currency ] ) );
			}
	    ...
	    public function get_booked_dates( $view = '' ) {
	    	...
	    	if ( ovabrw_array_exists( $part_day ) && 'hotel' === $this->product->get_charged_by() ) {
	    		// Half day
	    		$half_day = [];

	    		// Pick-up time
	    		$pickup_time = ovabrw_get_hotel_pickup_time();
	    		if ( strtotime( $pickup_time ) ) {
	    			$pickup_time = gmdate( 'H:i', strtotime( $pickup_time ) );
	    		}

	    		// Drop-off time
	    		$dropoff_time = ovabrw_get_hotel_dropoff_time();
	    		if ( strtotime( $dropoff_time ) ) {
	    			$dropoff_time = gmdate( 'H:i', strtotime( $dropoff_time ) );
	    		}

	    		// Get all item check-in (14:00)
	    		foreach ( $part_day as $i => $item_checkin ) {
	    			$checkin = strtotime( ovabrw_get_meta_data( 'start', $item_checkin ) );
	    			if ( !$checkin ) continue;

	    			// Check-in date
	    			$checkin_date = gmdate( $date_format, $checkin );

	    			// Check-in time
	    			$checkin_time = gmdate( 'H:i', $checkin );

	    			// Check is item check-in (14:00)
	    			if ( $checkin_time === $pickup_time ) {
	    				// Get all item check-out (11:00)
	    				foreach ( $part_day as $j => $item_checkout ) {
	    					$checkout = strtotime( ovabrw_get_meta_data( 'end', $item_checkout ) );
			    			if ( !$checkout ) continue;

			    			// Check-out date
			    			$checkout_date = gmdate( $date_format, $checkout );

			    			// Check same day
			    			if ( $checkin_date !== $checkout_date ) continue;

			    			// Check-out time
			    			$checkout_time = gmdate( 'H:i', $checkout );

			    			// Check is item check-out(11:00)
			    			if ( $checkout_time !== $dropoff_time ) continue;

			    			// Add half day
			    			if ( ovabrw_array_exists( $checkout_date, $half_day ) ) {
			    				$half_day[$checkout_date] += (int)ovabrw_get_meta_data( 'quantity', $item_checkout );
			    			} else {
			    				$half_day[$checkout_date] = (int)ovabrw_get_meta_data( 'quantity', $item_checkin ) + (int)ovabrw_get_meta_data( 'quantity', $item_checkout );
			    			}
	    		...
	    		// Check half day
	    		if ( ovabrw_array_exists( $half_day ) ) {
	    			foreach ( $half_day as $date => $quantity ) {
	    				$quantity /= 2;

	    				if ( $quantity >= $stock_qty && !in_array( $date, $full_day ) ) {
	    					array_push( $full_day, $date );
	    				}
	    			} // END loop
	    		}
	    	...
	    	if ( ovabrw_array_exists( $part_day ) ) {
	    		$part_day = array_values( $part_day );
	    ...
	    public function get_rental_time_between( $from_date, $end_date ) {
	    	...
	    	if ( $from_date && $end_date ) {
	    		$rental_time['numberof_rental_days'] = $this->get_number_of_rental_days( $from_date, $end_date );
	    	}
	    ...
	    public function get_new_date( $args = [] ) {
	    	...
	    	// Charged by hotel
	    	if ( 'hotel' === $this->product->get_charged_by() ) {
	    		// For hotel
	    		if ( $pickup_date && $pickup_date == $dropoff_date ) {
	    			$dropoff_date = (int)$dropoff_date + 86400;
	    		}

	    		// Pickup
	    		$pickup_date = $pickup_date ? strtotime( gmdate( 'Y-m-d', $pickup_date ) . ' ' . ovabrw_get_hotel_pickup_time() ) : '';

	    		// Dropoff
	    		$dropoff_date = $dropoff_date ? strtotime( gmdate( 'Y-m-d', $dropoff_date ) . ' ' . ovabrw_get_hotel_dropoff_time() ) : '';
	    	} else {
	    		// Pickup
		    	$pickup_date = $pickup_date ? strtotime( gmdate( $date_format, $pickup_date ) ) : '';

		    	// Dropoff
		    	$dropoff_date = $dropoff_date ? strtotime( gmdate( $date_format, $dropoff_date ) ) : '';
	    	}
	    ...
	    public function min_days_validation( $pickup_date, $dropoff_date ) {
	    	...
	    	if ( $min_days ) {
		    	...
		    	if ( $min_days > $numberof_rental_days ) {
		    		if ( 1 == $min_days ) {
		    			if ( 'hotel' === $charged_by ) {
		    				$mesg = sprintf( esc_html__( 'Min rental period: %s night', 'ova-brw' ), $min_days );
		    			} else {
		    				$mesg = sprintf( esc_html__( 'Min rental period: %s day', 'ova-brw' ), $min_days );
		    			}
		    		} else {
		    			if ( 'hotel' === $charged_by ) {
		    				$mesg = sprintf( esc_html__( 'Min rental period: %s nights', 'ova-brw' ), $min_days );
		    			} else {
		    				$mesg = sprintf( esc_html__( 'Min rental period: %s days', 'ova-brw' ), $min_days );
		    			}
		    		}
		    	}
	    	}
	    ...
	    public function max_days_validation( $pickup_date, $dropoff_date ) {
	    	...
	    	if ( $max_days ) {
	    		...
	    		if ( $max_days < $numberof_rental_days ) {
	    			if ( 1 == $max_days ) {
	    				if ( 'hotel' === $charged_by ) {
	    					$mesg = sprintf( esc_html__( 'Max rental period: %s night', 'ova-brw' ), $max_days );
	    				} else {
	    					$mesg = sprintf( esc_html__( 'Max rental period: %s day', 'ova-brw' ), $max_days );
	    				}
	    			} else {
	    				if ( 'hotel' === $charged_by ) {
	    					$mesg = sprintf( esc_html__( 'Max rental period: %s nights', 'ova-brw' ), $max_days );
	    				} else {
	    					$mesg = sprintf( esc_html__( 'Max rental period: %s days', 'ova-brw' ), $max_days );
	    				}
	    			}
	    		}
	    	}
	    ...
	    public function get_time_between_leases() {
	    	return apply_filters( $this->prefix.'get_time_between_leases', (float)$this->get_meta_value( 'prepare_vehicle_day' ) * 86400, $this );
	    }
	    ...
	    public function get_rental_calculations( $args = [] ) {
	    	...
	    	// Get special prices
	    	$special_prices = $this->get_special_prices( $pickup_date, $dropoff_date, $numberof_rental_days );
	    	...
	    	if ( $numberof_rental_days <= $special_qty ) {
				...
				// Get discount prices
				$discount_prices = $this->get_discount_prices( $numberof_rental_days - $special_qty );
				if ( $discount_prices ) {
					$daily_prices = [
	        ...
	        if ( $numberof_rental_days ) {
		        ...
		        if ( 'hotel' == $charged_by ) {
		        	$price_details['subtotal'] = sprintf(
		        		...
		        		$numberof_rental_days,
		        		ovabrw_wc_price( $subtotal )
		        	);
		        } else {
		        	$price_details['subtotal'] = sprintf(
		        		esc_html__( 'Number of days: %d — Cost: %s', 'ova-brw' ),
		        		$numberof_rental_days,
		        		ovabrw_wc_price( $subtotal )
	                );
		        }
	    ...
	    public function get_number_of_rental_days( $pickup_date, $dropoff_date ) {
	    	...
	    	// Pickup & Dropoff
	    	$new_pickup 	= $pickup_date;
	    	$new_dropoff 	= $dropoff_date;

	    	// Charged by
	    	$charged_by = $this->product->get_charged_by();

	    	if ( 'day' === $charged_by ) {
	    		$new_pickup 	= strtotime( gmdate( 'Y-m-d', $new_pickup ) );
	    		$new_dropoff 	= strtotime( gmdate( 'Y-m-d', $new_dropoff ) ) + 86400 - 1;
	    	} elseif ( 'hotel' === $charged_by ) {
	    		$new_pickup 	= strtotime( gmdate( 'Y-m-d', $new_pickup ) );
	    		$new_dropoff 	= strtotime( gmdate( 'Y-m-d', $new_dropoff ) );
	    	}

	    	// Number of rental days
	    	$numberof_rental_days = ceil( ( $new_dropoff - $new_pickup ) / 86400 );
	    	if ( !$numberof_rental_days && ( !$this->product->show_date_field( 'dropoff' ) || $pickup_date === $dropoff_date ) ) {
	    		$numberof_rental_days = 1;
	    	}
	     ...
	     * Get special prices
	     */
	    public function get_special_prices( $pickup_date, $dropoff_date, $numberof_rental_days ) {
	    	...
	    	if ( ovabrw_array_exists( $special_prices ) ) {
	    		...
	    		foreach ( $special_prices as $i => $price ) {
	    			...
	    			// Rental period
	    			$rental_period = $discount_prices = 0;
	    			...
	    			if ( $numberof_rental_days <= ( $results['quantity'] + $rental_period ) ) {
	    				...
	    				// Get discount prices
	    				$discount_prices = (float)$this->get_special_discount_prices( $discount, $price, $rental_period );

	    				$results['total'] 		+= $discount_prices * $rental_period;
	    	...
	    	return apply_filters( $this->prefix.'get_special_prices', $results, $pickup_date, $dropoff_date, $numberof_rental_days, $this );
	    }
	    ...
	    public function get_special_discount_prices( $discount, $price, $rental_period ) {
	    	...
	    	return apply_filters( $this->prefix.'get_special_discount_prices', $special_price, $discount, $price, $rental_period, $this );
	    }

	    /**
	     * Get discount price
	     */
	    public function get_discount_prices( $numberof_rental_days ) {
	    	...
	    	if ( ovabrw_array_exists( $disc_price ) ) {
	    		...
	    		foreach ( $disc_price as $i => $p ) {
	    			...
	    			if ( $numberof_rental_days >= $from && $numberof_rental_days <= $to ) {
	    				$price = (float)$p;
	    				break;
	    			}
	    		}
	    	...
	    	return apply_filters( $this->prefix.'get_discount_prices', $price, $numberof_rental_days, $this );
	    }
		...
		public function add_rental_cart_item_data( $cart_item_data, $product_id, $variation_id, $quantity ) {
	    	...
	    	// Charged by
	    	$cart_item_data['charged_by'] = $this->product->get_charged_by();
	    	...
	    	if ( !ovabrw_get_meta_data( 'dropoff_date', $cart_item_data ) ) {
		    	...
		    	if ( !$dropoff_date ) {
					...
					if ( 'hotel' === $charged_by ) {
						// Get new data
			    		$new_dates = $this->get_new_date([
			    			'pickup_date' => strtotime( $cart_item_data['pickup_date'] )
			    		]);

			    		if ( ovabrw_get_meta_data( 'dropoff_date', $new_dates ) ) {
			    			$dropoff_date = gmdate( OVABRW()->options->get_datetime_format(), ovabrw_get_meta_data( 'dropoff_date', $new_dates ) );
			    		}
					} else {
						$dropoff_date = $cart_item_data['pickup_date'];
					}
		    	}
		...
		public function get_real_data( $pickup_date, $dropoff_date ) {
			...
			if ( 'day' === $charged_by ) {
				// Pick-up real date
				$real_data['pickup_real'] = gmdate( $date_format, $pickup_date ) . ' 00:00';

				// Drop-off real date
				$real_data['dropoff_real'] = gmdate( $date_format, $dropoff_date ) . ' 24:00';
			} elseif ( 'hotel' === $charged_by ) {
				if ( $pickup_date === $dropoff_date ) {
					$dropoff_date += 86400;
				}
				...
				// Pick-up real date
				$real_data['pickup_real'] = gmdate( $date_format, $pickup_date ) . ' ' . ovabrw_get_hotel_pickup_time();

				// Drop-off real date
				$real_data['dropoff_real'] = gmdate( $date_format, $dropoff_date ) . ' ' . ovabrw_get_hotel_dropoff_time();
			} elseif ( 'hour' === $charged_by ) {
				// Date time format
				$datetime_format = OVABRW()->options->get_datetime_format();

				// Pick-up real date
				$real_data['pickup_real'] = gmdate( $datetime_format, $pickup_date );

				// Drop-off real date
				$real_data['dropoff_real'] = gmdate( $datetime_format, $dropoff_date );
			}
			...
			if ( ovabrw_array_exists( $special_prices ) ) {
				...
				foreach ( $special_prices as $i => $price ) {
					...
					if ( $rental_period > 0 ) {
						// Get discount prices
    					$discount_prices = (float)$this->get_special_discount_prices( $discounts, $price, $rental_period );
			...
			if ( $numberof_rental_days > 0 ) {
				...
				// Get discount prices
				$discount_prices = $this->get_discount_prices( $numberof_rental_days );
				if ( $discount_prices ) {
				...
				if ( 'hotel' === $charged_by ) {
					// Quantity real
					$real_data['quantity_real'] .= sprintf( esc_html__( '%s Night(s)', 'ova-brw' ) . '<br>', $numberof_rental_days );

					// Price real
					$real_data['price_real'] .= sprintf( esc_html__( '%s/Night', 'ova-brw' ) . '<br>', ovabrw_wc_price( wc_get_price_including_tax( $this->product, [ 'price' => $regular_price ] ) ) );
				} else {
					// Quantity real
		...
		public function new_booking_handle_item( $meta_key, $args, $order ) {
			...
			// Charged by
			$item_meta['define_day'] = $this->product->get_charged_by();
