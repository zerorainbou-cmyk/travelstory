...
/**
 * OVABRW Rental By Transportantion
 */
if ( !class_exists( 'OVABRW_Rental_By_Transportation' ) ) {

	class OVABRW_Rental_By_Transportation extends OVABRW_Rental_Types {
		...
		public function get_price_html( $price_html = '', $currency = '' ) {
			...
			// Price locations
            $price_locations = $this->get_meta_value( 'price_location' );

            if ( ovabrw_array_exists( $price_locations ) ) {
                $min = min( $price_locations );
                $max = max( $price_locations );
            }
	    ...
	    public function get_calendar_regular_price() {
			...
			if ( !ovabrw_array_exists( $location_prices ) ) return '';

			if ( 'highest' === $filter_price ) {
				$product_price = max( $location_prices );
			} elseif ( 'average' === $filter_price ) {
				$product_price = array_sum( $location_prices ) / count( $location_prices );
			} elseif ( 'lowest' === $filter_price ) {
				$product_price = min( $location_prices );
			}
	    ...
	    public function get_new_date( $args = [] ) {
	    	...
	    	// Check dates exists
	    	if ( !$pickup_date || !$pickup_location || !$dropoff_location ) return false;

	    	// Get drop-off date
	    	if ( !$dropoff_date ) {
	    		// Get location time
	    		$location_time 	= $this->get_location_time( $pickup_location, $dropoff_location );
	    		$dropoff_date 	= $pickup_date + $location_time * 60;
	    	}
	     ...
	     * Get location time
	     */
	    public function get_location_time( $pickup_location, $dropoff_location ) {
	    	...
	    	if ( ovabrw_array_exists( $location_data ) ) {
	    		foreach ( $location_data as $location ) {
	    			if ( $pickup_location === $location['pickup'] && $dropoff_location === $location['dropoff'] ) {
	    	...
	    	return apply_filters( $this->prefix.'get_location_time', $time, $pickup_location, $dropoff_location );
	    }

	    /**
	     * Get location price
	     */
	    public function get_location_price( $pickup_location, $dropoff_location ) {
	    	if ( !$pickup_location || !$dropoff_location ) return 0;
	    	...
	    	return apply_filters( $this->prefix.'get_location_price', $price, $pickup_location, $dropoff_location );
	    }
	    ...
	    public function get_location_data() {
	    	...
	    	if ( ovabrw_array_exists( $pickup_location ) ) {
	    		...
	    		foreach ( $pickup_location as $i => $pickup ) {
	    			if ( !$pickup ) continue;
	    			$pickup = trim( $pickup );

	    			// Dropoff
	    			$dropoff = trim( ovabrw_get_meta_data( $i, $dropoff_location ) );
	    			if ( !$dropoff ) continue;

	    			// Price
	    			$price = (float)ovabrw_get_meta_data( $i, $location_prices );

	    			// Time
	    			$time = (float)ovabrw_get_meta_data( $i, $location_times );
	    	...
	    	return apply_filters( $this->prefix.'get_location_data', $locations, $this );
	    }
	    ...
	    public function get_rental_calculations( $args = [] ) {
	    	...
	    	// Pick-up location
	    	$pickup_location = ovabrw_get_meta_data( 'pickup_location', $args );
	    	if ( !$pickup_location ) return 0;

	    	// Drop-off location
	    	$dropoff_location = ovabrw_get_meta_data( 'dropoff_location', $args );
	    	if ( !$dropoff_location ) return 0;

	    	// Get price
	    	$rental_price = $this->get_location_price( $pickup_location, $dropoff_location );
	    ...
	    public function get_price_details( $cart_item = [] ) {
	        ...
	        if ( apply_filters( OVABRW_PREFIX.'show_location_prices', true ) ) {
		        ...
		        // Get location price
		        $location_prices = $this->get_location_price( $pickup_location, $dropoff_location );
		        if ( $location_prices ) {
		...
		public function add_rental_cart_item_data( $cart_item_data, $product_id, $variation_id, $quantity ) {
	    	...
	    	if ( !ovabrw_array_exists( 'pickup_date', $cart_item_data ) || !ovabrw_array_exists( 'dropoff_date', $cart_item_data ) ) {
	    		// Pick-up date
		    	...
		    	$new_date = $this->get_new_date([
		    		'pickup_date' 		=> $pickup_date,
		    		'dropoff_date' 		=> $dropoff_date,
		    		'pickup_location' 	=> $cart_item_data['pickup_location'],
		    		'dropoff_location' 	=> $cart_item_data['dropoff_location']
		    	]);

		    	if ( !$new_date ) return $cart_item_data;
	    	...
	    	// Get location price
	    	$location_price = $this->get_location_price( $cart_item_data['pickup_location'], $cart_item_data['dropoff_location'] );

	    	// Price real
	    	$cart_item_data['price_real'] = $location_price ? ovabrw_wc_price( wc_get_price_including_tax( $this->product, [ 'price' => $location_price ] ) ) : 0;
		...
		public function get_request_booking_mail_content( $data = [] ) {
    		...
    		if ( ovabrw_array_exists( $new_date ) ) {
    			// Date format
    			$date_format = OVABRW()->options->get_datetime_format();

    			// Pick-up date
    			$pickup_date = gmdate( $date_format, ovabrw_get_meta_data( 'pickup_date', $new_date ) );

    			// Drop-off date
    			$dropoff_date = gmdate( $date_format, ovabrw_get_meta_data( 'dropoff_date', $new_date ) );
    		}
		...
		public function new_booking_handle_item( $meta_key, $args, $order ) {
			...
			if ( $location_prices ) {
				...
				// Price detail
				$item_meta['ovabrw_price_detail'] = str_replace( '<br>', ', ', $location_prices );
			}
	    	...
	    	$pickup_date = ovabrw_get_meta_data( 'pickup_date', $new_date );
			if ( $pickup_date ) {
				$item_meta['ovabrw_pickup_date'] 			= gmdate( $datetime_format, $pickup_date );
				$item_meta['ovabrw_pickup_date_strtotime'] 	= $pickup_date;

				// Pick-up date real
	    		$item_meta['ovabrw_pickup_date_real'] = $item_meta['ovabrw_pickup_date'];
			}

			// Drop-off date
			$dropoff_date = ovabrw_get_meta_data( 'dropoff_date', $new_date );
			if ( $dropoff_date ) {
				$item_meta['ovabrw_pickoff_date'] 			= gmdate( $datetime_format, $dropoff_date );
				$item_meta['ovabrw_pickoff_date_strtotime'] = $dropoff_date;

				// Drop-off date real
	    		$item_meta['ovabrw_pickoff_date_real'] = $item_meta['ovabrw_pickoff_date'];
			}
	    ...
	    public function get_add_to_cart_data() {
    		...
    		if ( !$this->location_validation( $pickup_location, $dropoff_location ) ) return false;
