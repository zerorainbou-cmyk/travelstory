...
if ( !class_exists( 'OVABRW_Rental_By_Taxi' ) ) {
	...
	class OVABRW_Rental_By_Taxi extends OVABRW_Rental_Types {
	    ...
	    public function get_price_details( $cart_item = [] ) {
	        ...
	        // Get distance
	        $distance = (float)ovabrw_get_meta_data( 'distance', $cart_item );
            ...
            if ( $extra_time ) {
            	...
            	if ( $extra_price ) {
            		...
            		$price_details['extra_time_prices'] = sprintf(
		                esc_html__( 'Extra time prices: %s', 'ova-brw' ),
		                ovabrw_wc_price( $extra_price * $quantity )
		            );
            	}
            }
	        ...
	        if ( $subtotal ) {
	            ...
	            // Get price unit and convert distance
	            $price_by = $this->get_meta_value( 'map_price_by' );
	            if ( 'km' === $price_by ) {
	                $distance_string = round( $distance / 1000, 2 );
	                $unit = 'km';
	            } elseif ( 'mi' === $price_by ) {
	                $distance_string = round( $distance / 1609.34, 2 );
	                $unit = 'mi';
	            }
	            
	            // Render price details
	            $price_details['subtotal'] = sprintf(
	                esc_html__( 'Distance: %.2f%s — Cost: %s', 'ova-brw' ),
	                $distance_string,
	                $unit,
	    ...
	    public function get_special_prices( $distance, $rental_price, $pickup_date ) {
	    	...
	    	return apply_filters( $this->prefix.'get_special_prices', $new_price, $distance, $rental_price, $pickup_date, $this );
	    }
