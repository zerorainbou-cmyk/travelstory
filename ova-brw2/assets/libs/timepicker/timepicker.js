(function () {
  ...
  var Timepicker = /*#__PURE__*/function () {
    ...
    _createClass(Timepicker, [{
      ...
      value: function parseSettings(settings) {
        ...
        settings.allowTimes = this._allowTimes(settings.allowTimes);

        if (settings.scrollSelect) {
          settings.scrollSelect = this.anytime2int(settings.scrollSelect);
        }
      ...
      key: "_allowTimes",
      value: function _allowTimes(allowTimes) {
        if (!allowTimes || allowTimes.length == 0) {
          return [];
        }
        ...
        for (var i in allowTimes) {
          var timeInt = this.anytime2int(allowTimes[i]);
          if ( null != timeInt ) {
            newAllowTimes[i] = timeInt;
          }
        }
  ...
  function _getDropdownTimes(tp) {
    ...
    // allowTimes
    var allowTimes = settings.allowTimes;

    var output = [];

    for (var i = start, j = 0; i <= end; j++, i += settings.step(j) * 60) {
      if ( allowTimes.length > 0 && !allowTimes.includes(i) ) {
          continue;
      }
  ...
  function _renderSelectItem(item, tp) {
    var el = document.createElement('option');
    el.value = item.value || item.label;

    if ( tp.settings.scrollSelect && tp.settings.scrollSelect == el.value ) {
      // Mark it as selected
      el.selected = true;
    }
  ...
  function _renderSelectList(items, targetName, tp) {
    ...
    try {
      for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
        ...
        var itemEl = _renderSelectItem(item, tp);
  ...
  function renderHtml(tp) {
    ...
    if (tp.settings.useSelect) {
      el = _renderSelectList(items, tp.targetEl.name, tp);
    } else {
  ...
  (function (factory) {
    ...
    var methods = {
      ...
      getTimeString: function() {
        ...
        if (isNaN(parseInt(time_string))) {
          return null;
        }

        var timeString = tp._int2time(parseInt(time_string));
