...
/**
 * Class OVABRW_Widget_Product_Rating
 */
if ( !class_exists( 'OVABRW_Widget_Product_Rating' ) ) {

	class OVABRW_Widget_Product_Rating extends \Elementor\Widget_Base {
		...
		protected function render() {
			...
			if ( 'modern' === ovabrw_get_meta_data( 'product_template', $settings ) ): ?>
				<div class="ovabrw-modern-product">
					<?php ovabrw_get_template( apply_filters( OVABRW_PREFIX.'widget_template_product_rating', 'modern/single/detail/ovabrw-product-review.php', $settings ), $settings ); ?>
				</div>
			<?php else: ?>
				<div class="elementor-rating">
					<?php wc_get_template( apply_filters( OVABRW_PREFIX.'widget_template_product_rating', 'single-product/rating.php', $settings ), [
	...
	// Register new widget
	$widgets_manager->register( new OVABRW_Widget_Product_Rating() );
}