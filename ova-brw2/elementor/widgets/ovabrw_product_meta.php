...
/**
 * Class OVABRW_Widget_Product_Meta
 */
if ( !class_exists( 'OVABRW_Widget_Product_Meta' ) ) {

	class OVABRW_Widget_Product_Meta extends \Elementor\Widget_Base {
			...
			$this->start_controls_section(
				...
				$this->add_control(
					...
					[
						...
						'selectors' => [
							'.woocommerce {{WRAPPER}} .posted_in' => 'color: {{VALUE}}',
						],
					]
				);
				...
				$this->add_group_control(
					...
					[
						'name' 	 	=> 'typography',
						'selector' 	=> '.woocommerce {{WRAPPER}} .posted_in',
					]
				);
				...
				$this->add_control(
					'category_meta_color',
					[
						'label' 	=> esc_html__( 'Color Category', 'ova-brw' ),
						'type' 		=> \Elementor\Controls_Manager::COLOR,
						'selectors' => [
							'.woocommerce {{WRAPPER}} .posted_in a' => 'color: {{VALUE}}',
						],
					]
				);
				...
				$this->add_responsive_control(
					'space_between',
					[
						'label' 	=> esc_html__( 'Space Between', 'ova-brw' ),
						...
						'selectors' => [
							'.woocommerce {{WRAPPER}} .posted_in a' => 'margin-right: calc({{SIZE}}{{UNIT}}/2); margin-left: calc({{SIZE}}{{UNIT}}/2)',
						],
					]
				);
		...
		protected function render() {
			...
			if ( 'modern' === ovabrw_get_meta_data( 'product_template', $settings ) ): ?>
				...
				<?php
					// Get product categories template
					ovabrw_get_template( apply_filters( OVABRW_PREFIX.'widget_template_product_categories', 'modern/single/detail/ovabrw-product-categories.php', $settings ), $settings );

					// Get product attributes template
					ovabrw_get_template( apply_filters( OVABRW_PREFIX.'widget_template_product_attributes', 'modern/single/detail/ovabrw-product-attributes.php', $settings ), $settings );
				?>
				</div>
			<?php else: ?>
				<div class="elementor-meta">
					<?php wc_get_template( apply_filters( OVABRW_PREFIX.'widget_template_product_meta', 'single-product/meta.php', $settings ), [
	...
	$widgets_manager->register( new OVABRW_Widget_Product_Meta() );
}