...
// Guest name
$guest_name = ovabrw_get_meta_data( 'guest_name', $args );

// Guest index
$key = (int)ovabrw_get_meta_data( 'key', $args );
    		...
    		<?php if ( ovabrw_array_exists( $guest_fields ) ):
    			...
    			<div class="guest-info-field ovabrw-guest-info-<?php echo esc_attr( $type ); ?>">
					<?php if ( in_array( $type, ['radio', 'checkbox'] ) ): ?>
    					<div class="label-field <?php echo esc_attr( $required_class ); ?>">
    				...
    				if ( 'textarea' === $type ) {
						ovabrw_textarea_input([
							'id' 			=> $field_id,
		                    'class'         => $class,
		                    'name'          => $product->get_meta_key( $guest_name.'_info['.$key.']['.$name.']' ),
		                    'default'       => $default,
						...
						ovabrw_radio_input([
		                    ...
		                    'class'         => $class,
		                    'name'          => $product->get_meta_key( $guest_name.'_info['.$key.']['.$name.']' ),
		                    'name_qty' 		=> $product->get_meta_key( $guest_name.'_info['.$key.']['.$name.'_qty]' ),
		                    'default'       => $default,
		                    'options'       => $options,
						...
						ovabrw_file_input([
							'id' 			=> $field_id,
		                    'class'         => $class,
		                    'name'          => $product->get_meta_key( $guest_name.'_'.$name.'['.$key.']' ),
		                    'default'       => $default,
		                    'required'      => $required,
		                    'attrs'         => [
		                        'accept'    => $accept,
