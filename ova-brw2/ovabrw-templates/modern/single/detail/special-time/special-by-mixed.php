...
// Special price by day
$special_price_day 	= $product->get_meta_value( 'rt_price' );

// Special price by hour
$special_price_hour = $product->get_meta_value( 'rt_price_hour' );

if ( ovabrw_array_exists( $special_price_day ) || ovabrw_array_exists( $special_price_hour ) ):
	// Date format
...
?>
	<div class="ovabrw-product-special-time">
		...
		<?php if ( ovabrw_array_exists( $special_price_day ) ): ?>
			<table class="ovabrw-table">
				...
				<tbody>
				<?php foreach ( $special_price_day as $k => $price ):
					// Start date
		...
		if ( ovabrw_array_exists( $special_price_hour ) ): ?>
			<table class="ovabrw-table">
				...
				<tbody>
				<?php foreach ( $special_price_hour as $k => $price ):
					// Start date
					$start_date = strtotime( ovabrw_get_meta_data( $k, $special_startdate ) );
					if ( !$start_date ) continue;
					$start = gmdate( $date_format, $start_date );
												...
												<?php if ( $dsc_price ): ?>
													<table class="ovabrw-table">
														...
														<?php foreach ( $dsc_price as $dsc_k => $dsc_v_price ):
															...
															if ( 'hours' === $dsc_v_type && $dsc_v_min != '' && $dsc_v_max != '' && $dsc_v_price != '' ): ?>
																<tr>
																	<td>
																		<span>
																			<?php echo esc_html( $dsc_v_min ); ?>
