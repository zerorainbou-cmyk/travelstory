...
// Min guests
$min_guests = (int)$this->get_meta_value( 'min_guest' );
...
foreach ( $guest_options as $guest_item ) {
    // Get guest price
    $price = (float)$this->get_meta_value( $guest_item['name'].'_price' );
    if ( $price ) $guest_item['price'] = $price;

    // Min
    $min = (int)$this->get_meta_value( 'min_'.$guest_item['name'] );
    $guest_item['min'] = $min;

    // Max
    $max = $this->get_meta_value( 'max_'.$guest_item['name'] );
    $guest_item['max'] = $max;
...
<div class="rental_item ovabrw-guests-field">
                    ...
                    <div class="guests-info">
                        ...
                        <?php if ( $show_price && $price ): ?>
                            <div class="guests-price <?php echo esc_attr( $name ); ?>-price">
                                <?php printf( esc_html__( '%s/guest', 'ova-brw' ), wp_kses_post( ovabrw_wc_price( $price, [ 'currency' => $currency ] ) ) ); ?>
                            </div>
                        <?php endif; ?>
                    </div>
